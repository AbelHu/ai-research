...
> Every payload schema is **pydantic-validated** at the role boundary (like skill params, §8.6); a malformed envelope or AI reply is repaired/retried or escalated, never acted on blindly. **This applies to every AI-facing role — explicitly including the experts (Company Expert, Plan Expert): each must validate that the model's reply conforms to its template's declared response schema (the *template requirement*) — required fields, correct types, and *no extra/invented fields* — before the verdict is used. A reply that doesn't meet the template requirement is treated as a hallucination and repaired or escalated, never acted on. Validation also extends to *content the model cites*: any **URL** carried in an answer's citations must be **deterministically verified to exist** (resolved + reachable) before the answer is accepted — the experts access the cited URLs to confirm they are real; an unreachable or fabricated URL is treated as a hallucination and the answer is repaired or escalated. The fetch is deterministic code (an SSRF-guarded existence check, never the AI), so AI-proposed URLs can't be used to reach internal resources (§7.1). This URL check is **configurable** (`verify_citation_urls`, default **on**) because anti-crawler defenses can make a real page unreachable to our fetcher — see §7.1.**
...
- **Cited URLs are verified to exist.** Before an answer is accepted, deterministic code **resolves and reaches every URL** carried in its citations; a URL that doesn't exist (or is unreachable) is treated as a **hallucination** and the answer is repaired or escalated — never returned with a fabricated link. The experts apply this same check when reviewing reports. The verifier is **SSRF-guarded** (only `http`/`https`; rejects URLs resolving to private/loopback/link-local/reserved addresses) so an AI-proposed URL can never be used to reach internal services or cloud metadata endpoints.
  - **Configurable (default on), because of anti-crawler defenses.** Some models/agents can open a page through means our **deterministic** fetcher can't: **CAPTCHAs, bot/JS challenges, paywalls, or geofencing** can make a perfectly **real** URL look unreachable — a **false negative** that would wrongly reject a good answer. A policy knob **`verify_citation_urls` (default `true`)** turns the existence check off when that trade-off bites; with it off, cited URLs are kept as **provenance** but not fetched. This is a deliberate **first cut to be hardened later** (e.g. treat ambiguous/blocked responses as a soft-pass, cross-check existence via the search API, or render headless) — not a permanent limitation. The knob lives with the other deterministic knobs (`config/policies.yaml`), so the AI never sets it.
...
5. **Draft reply** (advisor) from the hits → **validated** (must cite the memory it used; any cited **URL** is deterministically verified to exist — §7.1 — or the draft is repaired/escalated).
